...
const displayAnswers = (data) => {
	...
	data.forEach((answer, idx) => {
        ...
        <div class="flex justify-around">
          <p class="w-5 h-5 ${
						answer.options.indexOf(answer.givenAns) === 0 ? 'bg-orange-500 animate-pulse' : 'bg-gray-500'
					} rounded-full text-white flex justify-center items-center">
            1
          </p>
          <p class="w-5 h-5 ${
						answer.options.indexOf(answer.givenAns) === 1 ? 'bg-orange-500 animate-pulse' : 'bg-gray-500'
					} rounded-full text-white flex justify-center items-center">
            2
          </p>
          <p class="w-5 h-5 ${
						answer.options.indexOf(answer.givenAns) === 2 ? 'bg-orange-500 animate-pulse' : 'bg-gray-500'
					} rounded-full text-white flex justify-center items-center">
            3
          </p>
          <p class="w-5 h-5 ${
						answer.options.indexOf(answer.givenAns) === 3 ? 'bg-orange-500 animate-pulse' : 'bg-gray-500'
					} rounded-full text-white flex justify-center items-center">
            4
          </p>
        </div>
...
const showAnswers = (data) => {
	...
	data.forEach((answer, idx) => {
		quizContainer.innerHTML += `<div class="text-left">
          ...
          <div>
          <h1 class="text-sm">Given Answer: <span class="text-orange-600">${answer.givenAns}</span></h1>
          <h1 class="text-sm">Correct Answer: <span class="text-green-600">${answer.answer}</span></h1>
          <p class="text-[10px] mt-5"><span class="font-medium mr-1">Description:</span>${answer.description}</p>
          </div>
        </div>`;
	});
};
